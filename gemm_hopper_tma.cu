...
__device__ __forceinline__ void load_stage(
  ...
  asm volatile("mbarrier.arrive.expect_tx.shared::cta.b64 _, [%0], %1;"
               :: "r"(ba), "r"(TX_BYTES) : "memory");
  asm volatile(
      "cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes "
      "[%0], [%1, {%2, %3}], [%4];"
      :: "r"(shared_addr(sa)), "l"(a), "r"(k0), "r"(m0), "r"(ba) : "memory");
  asm volatile(
      "cp.async.bulk.tensor.2d.shared::cluster.global.mbarrier::complete_tx::bytes "
      "[%0], [%1, {%2, %3}], [%4];"
      :: "r"(shared_addr(sb)), "l"(b), "r"(n0), "r"(k0), "r"(ba) : "memory");
}
__device__ __forceinline__ void wait_stage(uint64_t* bar, int phase) {
  asm volatile(
      ...
      "@!p bra TMA_WAIT;\n"
      "}\n" :: "r"(shared_addr(bar)), "r"(phase) : "memory");
}
...
void wgmma_gemm_tma(const __grid_constant__ CUtensorMap mapA,
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < 5; ++i) wgmma_gemm_tma<<<grid, block>>>(mapA, mapB, dC);
  ...
  for (int i = 0; i < kIters; ++i) wgmma_gemm_tma<<<grid, block>>>(mapA, mapB, dC);
  CUDA_CHECK(cudaGetLastError());
